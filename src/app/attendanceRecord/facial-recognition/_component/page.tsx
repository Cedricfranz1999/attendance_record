...
const page = () => {
  ...
  const initializeFaceMatcher = async () => {
    try {
      ...
      const labeledFaceDescriptors = await Promise.all(
        data.data.map(async (user: any) => {
          try {
            ...
            if (detection) {
              return new faceapi.LabeledFaceDescriptors(`${user.id}_${user.firstname}_${user.lastName}`, [
      ...
      if (validDescriptors.length > 0) {
        console.log(`Successfully created face matcher with ${validDescriptors.length} descriptors`)
  ...
  const startVideo = async (videoRef: React.RefObject<HTMLVideoElement>, deviceId: string) => {
  ...
  const handleTimeInVideoOnPlay = () => {
    ...
    if (!modelsLoaded || !faceMatcher) {
      console.log("Models or face matcher not loaded yet. Cannot start processing.")
    ...
    const processTimeInVideo = async () => {
      ...
      try {
        if (videoReadyTimeIn && videoRefTimeIn.current && canvasRefTimeIn.current && faceMatcher) {
          const displaySize = { width: videoWidth, height: videoHeight }
          faceapi.matchDimensions(canvasRefTimeIn.current, displaySize)

          const singleResult = await faceapi
            .detectSingleFace(videoRefTimeIn.current, new faceapi.SsdMobilenetv1Options())
            .withFaceLandmarks()
            .withFaceDescriptor()
          ...
          if (singleResult) {
            ...
            if (bestMatch.distance < THRESHOLD) {
              ...
              // Record time in
              if (id) {
                console.log(id, "TIME IN")
              }
            } else {
              const box = singleResult.detection.box
              const resizedBox = new faceapi.draw.DrawBox(
                { x: box.x - 50, y: box.y - 50, width: box.width, height: box.height },
                { label: `Unknown` },
              )
              resizedBox.draw(canvasRefTimeIn.current)
            }
  ...
  const handleTimeOutVideoOnPlay = () => {
    ...
    const processTimeOutVideo = async () => {
      ...
      try {
        if (videoReadyTimeOut && videoRefTimeOut.current && canvasRefTimeOut.current && faceMatcher) {
          const displaySize = { width: videoWidth, height: videoHeight }
          faceapi.matchDimensions(canvasRefTimeOut.current, displaySize)

          const singleResult = await faceapi
            .detectSingleFace(videoRefTimeOut.current, new faceapi.SsdMobilenetv1Options())
            .withFaceLandmarks()
            .withFaceDescriptor()
  ...
  useEffect(() => {
    ...
    const setupCamerasAndListeners = async () => {
      ...
      return () => {
        ...
        if (videoElementTimeIn) {
          videoElementTimeIn.removeEventListener("canplay", handleCanPlayTimeIn)
        ...
        if (videoElementTimeOut) {
          videoElementTimeOut.removeEventListener("canplay", handleCanPlayTimeOut)
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-full bg-white p-4">
      <h1 className="text-2xl font-bold mb-8">Facial Recognition Attendance System</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-6xl">
        {/* Time In Camera */}
        <div className="flex flex-col items-center">
          <h2 className="text-xl font-semibold mb-2">Time In</h2>

          {/* Camera selector */}
          <div className="w-full max-w-xs mb-4">
            <Select value={timeInCameraId} onValueChange={handleTimeInCameraChange}>
              <SelectTrigger>
          ...
          <div className="flex flex-col items-center justify-center gap-4 text-center mb-2">
            <span className="text-[16px] text-black">{initializing ? "Loading Models..." : statusTimeIn}</span>
          </div>
          ...
          {detectTimeIn && (
            <div className="mt-4 p-3 bg-green-100 border border-green-300 rounded">
              ...
              <p className="text-sm text-green-700">Time In recorded successfully</p>
            </div>
          )}
        </div>
        ...
        {/* Time Out Camera */}
        <div className="flex flex-col items-center">
          <h2 className="text-xl font-semibold mb-2">Time Out</h2>

          {/* Camera selector */}
          <div className="w-full max-w-xs mb-4">
            <Select value={timeOutCameraId} onValueChange={handleTimeOutCameraChange}>
              <SelectTrigger>
          ...
          <div className="flex flex-col items-center justify-center gap-4 text-center mb-2">
            <span className="text-[16px] text-black">{initializing ? "Loading Models..." : statusTimeOut}</span>
          </div>
          <div className="relative flex items-center justify-center">
            <video
              ref={videoRefTimeOut}
          ...
          {detectTimeOut && (
            <div className="mt-4 p-3 bg-blue-100 border border-blue-300 rounded">
              ...
              <p className="text-sm text-blue-700">Time Out recorded successfully</p>
            </div>
          )}
        </div>
      </div>
